...
namespace UserActor
{
    ...
    internal class UserActor : Actor, IUserActor
    {
        ...
        public async Task AddToBasket(Guid productId, int qty)
        {
            await StateManager.AddOrUpdateStateAsync(productId.ToString(), qty, (id, oldQty) => oldQty + qty);
        }
        ...
        public async Task<Dictionary<Guid, int>> GetBasket()
        {
            ...
            foreach (var productId in productIds)
            {
                var qty = await StateManager.GetStateAsync<int>(productId);
                result[new Guid(productId)] = qty;
            }
            return result;
        }
